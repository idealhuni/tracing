#!/usr/bin/env python3
"""Ablation: Isotropic FMM traceback (vs Riemannian) -> neurons_iso_ablation.swc

Riemannian과 동일한 tubularity/파라미터를 사용하되, metric tensor의
방향 성분(GAMMA anisotropy)을 제거하고 scalar cost만 사용.
나머지(tip detection, traceback, pruning)는 완전히 동일.
"""
import argparse
import gc
import time
...
def main():
    ...
    OUT_SWC   = out_dir / 'neurons_iso_ablation.swc'
    ...
    # ── Isotropic scalar cost (ablation) ────────────────────────
    # Riemannian과 동일한 T 기반 cost를 사용하되, 방향 정보(GAMMA anisotropy) 제거.
    # cost = exp(-ALPHA * T): T가 높을수록 이동이 싸다 (= 빠르다).
    # Riemannian의 tube-perpendicular cost와 동일한 스케일.
    t0 = time.time()
    T64  = T_down.astype(np.float64)
    cost = np.exp(-ALPHA * T64)
    del T64; gc.collect()
    print(f'Isotropic cost built in {time.time()-t0:.1f}s  '
          f'range=[{cost.min():.4f}, {cost.max():.4f}]')
    print(f'(Riemannian anisotropy ratio would have been '
          f'{1.0 / (SIGMA_PERP * (1.0 - GAMMA * float(T_down.max()))):.1f}:1 — removed)')
    ...
    # ── Save SWC ─────────────────────────────────────────────────
    header = [
        f'# tracer_aniso ABLATION — Isotropic FMM (no directional anisotropy)',
        f'# ALPHA={ALPHA}  MIN_T_TIP={MIN_T_TIP}  (GAMMA/SIGMA_PERP not used)',
